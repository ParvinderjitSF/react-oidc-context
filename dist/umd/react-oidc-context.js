...
var AuthProvider = (props) => {
  ...
  const userManagerRef = import_react2.default.useRef(null);
  const createdInternally = import_react2.default.useRef(false);
  if (!userManagerRef.current) {
    if (userManagerProp) {
      userManagerRef.current = userManagerProp;
      createdInternally.current = false;
    } else {
      userManagerRef.current = UserManagerImpl ? new UserManagerImpl(
        userManagerSettings
  ...
  import_react2.default.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        user = user != null ? user : await userManager.getUser();
        dispatch({ type: "INITIALISED", user });
      } catch (error) {
        dispatch({ type: "ERROR", error: signinError(error) });
      }
      try {
        if (matchSignoutCallback == null ? void 0 : matchSignoutCallback(userManager.settings)) {
  ...
  import_react2.default.useEffect(() => {
    ...
    const handleSilentRenewError = (error) => dispatch({ type: "ERROR", error: renewSilentError(error) });
    ...
    return () => {
      ...
      if (createdInternally.current) {
        (_a = userManager.stopSilentRenew) == null ? void 0 : _a.call(userManager);
        (_b = userManager.clearStaleState) == null ? void 0 : _b.call(userManager);
      }
    };
  }, [userManager]);
